...
public class ManagementController {
    ...
    @Autowired
    public ManagementController(UserService userService, ReservationService reservationService, Util util) {
    ...
    public String showReservations(Model model, RedirectAttributes redirectAttributes, HttpSession session) {
        if (!authenticate(session, redirectAttributes)) return "redirect:/";
    ...
    public String deleteReservation(RedirectAttributes redirectAttributes, HttpSession session, @PathVariable int id) {
        val reservation = reservationService.findReservationById(id).get();
        if (!util.isStaff(session) && !reservation.getCustomer().getId().equals(session.getAttribute("id"))) {
        ...
        redirectAttributes.addFlashAttribute("alertClass", "success");
        return util.isStaff(session) ? "redirect:/management" : "redirect:/userinfo";
    }
    ...
    private boolean authenticate(HttpSession session, RedirectAttributes redirectAttributes) {
